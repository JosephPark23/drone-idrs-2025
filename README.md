...
**This brings me to my second reason, one I considered before pursuing the IDRS phase of this project**. One can dream (and dream big), but realistically, this IDRS is not going to be adopted *en masse* by adoring DJI fans; more likely is that the code will sit here and I will have spent ten hours organizing disparate files for nothing. But establishing that solutions to drone security exist - even if the implementation is clunky and rough in parts - is at least a step fowards for drone security. As far as I can tell, no research has yet been published on a live-tested IDRS for DJI drone Wi-Fi networks, (and extremely little research exists in this area in the first place). Novelty should of course never be pursued for novelty's sake; some sort of significant impact should be the first prerequisite for a research undertaking. Thus, I hope that part of this project's impact comes from that same issue of future drone security, where the sooner we start, the better off future society will be. 
